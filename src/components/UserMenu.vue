<template>
  <div v-on:click="isVisible = !isVisible" class="menu__user flex items-center justify-around absolute rounded-full cursor-pointer p-3">
    ...
    <div class="hidden lg:inline-block">
      <p class="font-bold">David Sanchez</p>
      <p class="text-gray-600">@sanchezdav</p>
    </div>
      ...
      <div v-show="isVisible" class="menu__user__submenu absolute p-3">
        <div class="relative py-1 bg-white border border-gray-200 rounded-md shadow-xl">
          ...
          <div class="relative">
            ...
            <a href="/" class="block w-full px-4 py-2 font-medium text-gray-700 whitespace-no-wrap hover:bg-gray-100 focus:outline-none hover:text-gray-900 focus:text-gray-900 focus:shadow-outline transition duration-300 ease-in-out">Sign Out</a>
          </div>
        </div>
      </div>
    </transition>
